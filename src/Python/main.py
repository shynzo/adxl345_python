	...
	while True:    
		try:
			...
			try:
				...
				print("AcX: {}m/s² | AcY: {}m/s² | AcZ: {}m/s²".format(entrada[0], entrada[1], entrada[2]))
			...
			try:
				x = np.vstack((acelX,acelY,acelZ))
				with open(os.path.join('data', arquivo), "w") as f:
					np.savetxt(f, x.transpose(), delimiter=';', fmt='%1.4f', header='AcX; AcY; AcZ') #Salvando o arquivo
			except Exception as e:
